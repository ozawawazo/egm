...
function sample(d){
    ...
    var node, anode, morpheme, startx, starty, endx, endy, end2x, end2y, maker;
    ...
    node.forEach(function(dd,i){
	    upedge[senbox[dd].number].forEach(function(ddd){
		    ...
		    senbox[snumber[ddd]].words.forEach(function(d4){
                            end2x = document.getElementsByName(d4)[0].transform.baseVal[0].matrix.e;
                            end2y = document.getElementsByName(d4)[0].transform.baseVal[0].matrix.f;
                            svg.append("path")  // パスを追加
                                .attr("d", line([[end2x + w/2, end2y + h/2], [startx + w/2, starty + h/2]]))   // 配列の座標を渡してpath要素のd属性に設定
                                .attr("stroke", "red")    // 線の色を黒にする
                                .attr("fill", "none")   // 線の塗りを無しにする
                                // pathの属性として、上で定義した矢印を指定します
                                .attr('marker-end', "url(#arrowhead)");
